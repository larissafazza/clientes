...
<div class="body-container">
    <form class="form" >
        ...
        <div class="mb-3">
            <img src="{{ asset($client->image_path) }}" alt="Imagem do Cliente">
        </div>
        <div class="mb-3">
            <label for="name" class="form-label">Nome</label>
            <input value="{{ $client->name }}" type="text" class="form-control" id="name" name="name" aria-describedby="name" disabled>
        </div>
        <div class="mb-3">
            <label for="mail" class="form-label">Email</label>
            <input value="{{ $client->mail }}" type="email" class="form-control" id="mail" name="mail" aria-describedby="emailHelp" disabled>
        </div>
        <div class="mb-3">
            <label for="person_type_id" class="form-label">Tipo de Pessoa</label>
            <select class="form-select" name="person_type_id" aria-label="Default select example" disabled>
        ...
        <div class="mb-3">
            ...
            @foreach ($sellers as $seller)
                <div class="form-check">
                    <input type="checkbox" class="form-check-input" id="seller{{ $seller->id }}" name="sellers[]" value="{{ $seller->id }}" {{ in_array($seller->id, $client->sellers->pluck('id')->toArray()) ? 'checked' : '' }} disabled>
        ...
        <div class="mb-3 phones">
            ...
            @foreach ($client->phones as $phone)
                <input type="phone" class="form-control mb-3" name="phones[]" value="{{ $phone->number }}" aria-describedby="phone" disabled>
            @endforeach
        </div>
